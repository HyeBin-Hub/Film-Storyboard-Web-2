...
if "RUNCOMFY_API_KEY" in st.secrets:
    ...
    deployment_id = st.sidebar.text_input("Deployment ID or Inference URL")
...
# CSV 업로드 / RunComfy parser test 상태
...
if "storyboard_csv_text" not in st.session_state:
    st.session_state.storyboard_csv_text = ""

if "csv_parser_test_result" not in st.session_state:
    st.session_state.csv_parser_test_result = None
...
with tab1:
    ...
    if st.session_state.step == 1:
        ...
        # =====================================================
        # Storyboard CSV Upload + RunComfy Parser Test
        # =====================================================
        ...
        if uploaded_csv is not None:
            ...
            if csv_text:
                st.session_state.storyboard_csv_text = csv_text
                ...
                st.error("CSV 파일을 utf-8-sig 또는 cp949 인코딩으로 읽지 못했습니다.")

        # -----------------------------------------------------
        # RunComfy CSV Parser Test
        # csv_text를 RunComfy /inference overrides로 전달
        # -----------------------------------------------------
        st.markdown("#### RunComfy CSV Parser Test")

        shot_filter = st.selectbox(
            "Shot Filter",
            ["ALL", "CUSTOM"],
            index=0,
            key="csv_parser_test_shot_filter"
        )

        custom_shot_ids = st.text_input(
            "Custom Shot IDs",
            value="",
            placeholder="예: 7-1,7-2",
            key="csv_parser_test_custom_ids"
        )

        if st.button(
            "🧪 TEST CSV → RunComfy Parser",
            use_container_width=True,
            key="test_csv_to_runcomfy_parser"
        ):
            csv_text_to_send = st.session_state.get("storyboard_csv_text", "")

            if not csv_text_to_send.strip():
                st.error("먼저 CSV 파일을 업로드해야 합니다.")
            elif shot_filter == "CUSTOM" and not custom_shot_ids.strip():
                st.error("CUSTOM 모드에서는 Custom Shot IDs를 입력해야 합니다.")
            else:
                try:
                    with st.spinner("Sending CSV text to RunComfy CSVStoryboardParser..."):
                        result = backend.test_csv_parser(
                            api_key=api_key,
                            deployment_id=deployment_id,
                            csv_text=csv_text_to_send,
                    ...
                    st.success("RunComfy request submitted.")
                    st.json(result)

                except Exception as e:
                    st.session_state.csv_parser_test_result = None
                    st.error(f"CSV parser test failed: {e}")

        if st.session_state.get("csv_parser_test_result") is not None:
            with st.expander("Last RunComfy CSV Parser Test Result"):
                st.json(st.session_state.csv_parser_test_result)
        ...
        with col_right:
            ...
            with st.expander("Advanced Setting", expanded=True):
                ...
                shots_per_character = st.slider(
                    "Shots per Character",
                    1,
                    4,
                    2,
                    key="shots_per_character"
                )
                ...
                st.markdown(
                    "<hr style='margin:6px 0; border:10; border-top:1px solid #333;'>",
                    unsafe_allow_html=True
                )
            ...
            with st.expander("Portrait Setting", expanded=True):
                with st.expander("Gender & Nationality"):
                    ...
                    pm_options["Gender"] = st.selectbox(
                        "Gender",
                        gender_options,
                        index=gender_options.index(pm_options.get("Gender", "Man")),
                        key=f"gender_{cur}",
                    )

                    nat_list = [
                        "Chinese", "Japanese", "Korean", "South Korean",
                        "Indian", "Saudi", "British", "French", "German",
                        "Italian", "Spanish", "American", "Canadian",
                        "Brazilian", "Mexican", "Argentine", "Egyptian",
                        "South African", "Nigerian", "Kenyan", "Moroccan",
                        "Australian", "New Zealander", "Fijian", "Samoan",
                        "Tongan"
                    ]
                ...
                with st.expander("Face & Body Type"):
                    face_shapes = [
                        ...
                        "Triangle", "Inverted Triangle", "Pear",
                        "Rectangle", "Oblong", "Long"
                    ]
                    ...
                    body_types = [
                        "Chubby", "Curvy", "Fat", "Fit", "Hefty", "Large",
                        "Lanky", "Muscular", "Obese", "Overweight", "Petite",
                        "Plump", "Short", "Skinny", "Slight", "Slim", "Small",
                        "Stout", "Stocky", "Tall", "Thick", "Tiny",
                        "Underweight", "Well-built"
                    ]
                ...
                with st.expander("Eyes Type"):
                    eye_colors = [
                        "Albino", "Amber", "Blue", "Brown", "Green",
                        "Gray", "Hazel", "Heterochromia", "Red", "Violet"
                    ]
                    ...
                    eye_shapes = [
                        "Almond Eyes Shape", "Asian Eyes Shape",
                        "Close-Set Eyes Shape", "Deep Set Eyes Shape",
                        "Downturned Eyes Shape", "Double Eyelid Eyes Shape",
                        "Hooded Eyes Shape", "Monolid Eyes Shape",
                        "Oval Eyes Shape", "Protruding Eyes Shape",
                ...
                with st.expander("Lips Type"):
                    lips_colors = [
                        "Berry Lips", "Black Lips", "Blue Lips", "Brown Lips",
                        "Burgundy Lips", "Coral Lips", "Glossy Red Lips",
                        "Mauve Lips", "Orange Lips", "Peach Lips", "Pink Lips",
                        "Plum Lips", "Purple Lips", "Red Lips", "Yellow Lips"
                    ]
                    ...
                    lips_shapes = [
                        "Full Lips", "Thin Lips", "Plump Lips", "Small Lips",
                        "Large Lips", "Wide Lips", "Round Lips",
                        "Heart-shaped Lips", "Cupid's Bow Lips"
                    ]
                ...
                with st.expander("Hair Style"):
                    hair_styles = [
                        "Bald", "Buzz", "Crew", "Pixie", "Bob", "Long bob",
                        "Long straight", "Wavy", "Curly", "Afro",
                        "Faded afro", "Braided", "Box braids", "Cornrows",
                        "Dreadlocks", "Pigtails", "Ponytail", "High ponytail",
                        "Bangs", "Curtain bangs", "Side-swept bangs",
                        "Mohawk", "Faux hawk", "Undercut", "Pompadour",
                        "Quiff", "Top Knot", "Bun", "Updo"
                    ]
                    ...
                    hair_colors = [
                        "Black", "Jet Black", "Blonde", "Platinum", "Brown",
                        "Chestnut", "Auburn", "Red", "Strawberry", "Gray",
                        "Silver", "White", "Salt and pepper"
                    ]
        ...
        disabled_next = (
            cur == num_characters - 1
        ) or (
            st.session_state.selected_face_urls[cur] is None
        )
    ...
    if st.session_state.step == 2:
        ...
        for char_i in range(n):
            ...
            with col_outfit:
                ...
                default_outfit = (
                    st.session_state.outfit_prompts[char_i]
                    or "white t-shirt, black pants, yellow sneakers"
                )
                ...
                if st.button(
                    ...
                    try:
                        with st.spinner("Fitting room... \n (Switch Mode: 2)"):
                            ...
                            if st.session_state.get("seed_mode") == "Fixed":
                                fixed_seed = st.session_state.get("fixed_seed", 12345)
                                seed_value = backend.derive_seed(fixed_seed + 1000, char_i)
        ...
        for i in range(n):
            with cols[i]:
                url = (
                    st.session_state.final_character_urls[i]
                    if i < len(st.session_state.final_character_urls)
                    else None
                )
